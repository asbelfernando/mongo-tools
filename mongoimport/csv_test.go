...
func TestCSVStreamDocument(t *testing.T) {
	...
	Convey("With a CSV input reader", t, func() {
		Convey("badly encoded CSV should result in a parsing error", func() {
			...
			r := NewCSVInputReader(colSpecs, bytes.NewReader([]byte(contents)), 1)
		...
		Convey("nested CSV fields should be imported properly", func() {
			...
			colSpecs := []ColumnSpec{
				{"a", new(FieldAutoParser), PG_AUTO_CAST},
				{"b.c", new(FieldAutoParser), PG_AUTO_CAST},
				{"c", new(FieldAutoParser), PG_AUTO_CAST},
			}
			expectedRead := bson.D{
				bson.DocElem{"a", 1},
		...
		Convey("valid CSV input file that starts with the UTF-8 BOM should "+
			...
			So(err, ShouldBeNil)
			r := NewCSVInputReader(colSpecs, fileHandle, 1)
...
func TestCSVReadAndValidateHeader(t *testing.T) {
	...
	Convey("With a CSV input reader", t, func() {
		...
		Convey("setting the header that ends in a dot should error", func() {
			...
			colSpecs := []ColumnSpec{}
			So(err, ShouldBeNil)
			So(NewCSVInputReader(colSpecs, bytes.NewReader([]byte(contents)), 1).ReadAndValidateHeader(), ShouldNotBeNil)
		})
