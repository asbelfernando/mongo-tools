...
func TestTypedHeaderParser(t *testing.T) {
	...
	Convey("Using 'zip.string(),number.double(),foo.auto()'", t, func() {
		...
		Convey("with parse grace: auto", func() {
			colSpecs, err = ParseTypedHeaders(headers, PG_AUTO_CAST)
			So(colSpecs, ShouldResemble, []ColumnSpec{
				{"zip", new(FieldStringParser), PG_AUTO_CAST},
				{"number", new(FieldDoubleParser), PG_AUTO_CAST},
				{"foo", new(FieldAutoParser), PG_AUTO_CAST},
				{"bar", &FieldDateParser{"January 2, 2006"}, PG_AUTO_CAST},
			})
			So(err, ShouldBeNil)
		})
		Convey("with parse grace: skipRow", func() {
			colSpecs, err = ParseTypedHeaders(headers, PG_SKIP_ROW)
			So(colSpecs, ShouldResemble, []ColumnSpec{
				{"zip", new(FieldStringParser), PG_SKIP_ROW},
				{"number", new(FieldDoubleParser), PG_SKIP_ROW},
				{"foo", new(FieldAutoParser), PG_SKIP_ROW},
				{"bar", &FieldDateParser{"January 2, 2006"}, PG_SKIP_ROW},
			})
			So(err, ShouldBeNil)
		})
	})
	...
	Convey("Using various bad headers", t, func() {
		...
		Convey("with non-empty arguments for types that don't want them", func() {
			columnSpec, err = ParseTypedHeader("zip.string(blah)", PG_AUTO_CAST)
			So(err, ShouldNotBeNil)
			columnSpec, err = ParseTypedHeader("zip.string(0)", PG_AUTO_CAST)
			So(err, ShouldNotBeNil)
			columnSpec, err = ParseTypedHeader("zip.int32(0)", PG_AUTO_CAST)
			So(err, ShouldNotBeNil)
			columnSpec, err = ParseTypedHeader("zip.int64(0)", PG_AUTO_CAST)
			So(err, ShouldNotBeNil)
			columnSpec, err = ParseTypedHeader("zip.double(0)", PG_AUTO_CAST)
			So(err, ShouldNotBeNil)
			columnSpec, err = ParseTypedHeader("zip.auto(0)", PG_AUTO_CAST)
			So(err, ShouldNotBeNil)
		})
		Convey("with bad arguments for the binary type", func() {
			columnSpec, err = ParseTypedHeader("zip.binary(blah)", PG_AUTO_CAST)
			So(err, ShouldNotBeNil)
			columnSpec, err = ParseTypedHeader("zip.binary(binary)", PG_AUTO_CAST)
			So(err, ShouldNotBeNil)
			columnSpec, err = ParseTypedHeader("zip.binary(decimal)", PG_AUTO_CAST)
			So(err, ShouldNotBeNil)
		})
	})
}
...
func TestAutoHeaderParser(t *testing.T) {
	Convey("Using 'zip,number'", t, func() {
		...
		So(colSpecs, ShouldResemble, []ColumnSpec{
			{"zip", new(FieldAutoParser), PG_AUTO_CAST},
			{"number", new(FieldAutoParser), PG_AUTO_CAST},
			{"foo", new(FieldAutoParser), PG_AUTO_CAST},
		})
	})
}
...
func TestFieldParsers(t *testing.T) {
	...
	Convey("Using FieldAutoParser", t, func() {
		var p, _ = NewFieldParser(CT_AUTO, "")
		var value interface{}
		var err error
	...
	Convey("Using FieldBooleanParser", t, func() {
		var p, _ = NewFieldParser(CT_BOOLEAN, "")
		var value interface{}
		var err error
	...
	Convey("Using FieldBinaryParser", t, func() {
		...
		Convey("using hex encoding", func() {
			var p, _ = NewFieldParser(CT_BINARY, "hex")
		...
		Convey("using base32 encoding", func() {
			var p, _ = NewFieldParser(CT_BINARY, "base32")
		...
		Convey("using base64 encoding", func() {
			var p, _ = NewFieldParser(CT_BINARY, "base64")
	...
	Convey("Using FieldDateParser", t, func() {
		...
		Convey("with Go's format", func() {
			var p, _ = NewFieldParser(CT_DATE_GO, "01/02/2006 3:04:05pm MST")
		...
		Convey("with MS's format", func() {
			var p, _ = NewFieldParser(CT_DATE_MS, "MM/dd/yyyy h:mm:sstt")
			Convey("parses valid timestamps correctly", func() {
		...
		Convey("with Oracle's format", func() {
			var p, _ = NewFieldParser(CT_DATE_ORACLE, "mm/Dd/yYYy hh:MI:SsAm")
			Convey("parses valid timestamps correctly", func() {
	...
	Convey("Using FieldDoubleParser", t, func() {
		var p, _ = NewFieldParser(CT_DOUBLE, "")
		var value interface{}
		var err error
	...
	Convey("Using FieldInt32Parser", t, func() {
		var p, _ = NewFieldParser(CT_INT_32, "")
		var value interface{}
		var err error
	...
	Convey("Using FieldInt64Parser", t, func() {
		var p, _ = NewFieldParser(CT_INT_64, "")
		var value interface{}
		var err error
	...
	Convey("Using FieldStringParser", t, func() {
		var p, _ = NewFieldParser(CT_STRING, "")
		var value interface{}
		var err error
