...
var (
	...
	csvConverters = []CSVConverter{
		CSVConverter{
			colSpecs: []ColumnSpec{
				{"field1", new(FieldAutoParser), PG_AUTO_CAST},
				{"field2", new(FieldAutoParser), PG_AUTO_CAST},
				{"field3", new(FieldAutoParser), PG_AUTO_CAST},
			},
			data:  []string{"a", "b", "c"},
			index: index,
		},
		CSVConverter{
			colSpecs: []ColumnSpec{
				{"field4", new(FieldAutoParser), PG_AUTO_CAST},
				{"field5", new(FieldAutoParser), PG_AUTO_CAST},
				{"field6", new(FieldAutoParser), PG_AUTO_CAST},
			},
			data:  []string{"d", "e", "f"},
			index: index,
		},
		CSVConverter{
			colSpecs: []ColumnSpec{
				{"field7", new(FieldAutoParser), PG_AUTO_CAST},
				{"field8", new(FieldAutoParser), PG_AUTO_CAST},
				{"field9", new(FieldAutoParser), PG_AUTO_CAST},
			},
			data:  []string{"d", "e", "f"},
			index: index,
		},
		CSVConverter{
			colSpecs: []ColumnSpec{
				{"field10", new(FieldAutoParser), PG_AUTO_CAST},
				{"field11", new(FieldAutoParser), PG_AUTO_CAST},
				{"field12", new(FieldAutoParser), PG_AUTO_CAST},
			},
			data:  []string{"d", "e", "f"},
			index: index,
		},
		CSVConverter{
			colSpecs: []ColumnSpec{
				{"field13", new(FieldAutoParser), PG_AUTO_CAST},
				{"field14", new(FieldAutoParser), PG_AUTO_CAST},
				{"field15", new(FieldAutoParser), PG_AUTO_CAST},
			},
			data:  []string{"d", "e", "f"},
			index: index,
		},
...
func TestTokensToBSON(t *testing.T) {
	...
	Convey("Given an slice of column specs and tokens to convert to BSON", t, func() {
		Convey("the expected ordered BSON should be produced for the given"+
			...
			colSpecs := []ColumnSpec{
				{"a", new(FieldAutoParser), PG_AUTO_CAST},
				{"b", new(FieldAutoParser), PG_AUTO_CAST},
				{"c", new(FieldAutoParser), PG_AUTO_CAST},
			}
		...
		Convey("fields with nested values should be set appropriately", func() {
			colSpecs := []ColumnSpec{
				{"a", new(FieldAutoParser), PG_AUTO_CAST},
				{"b", new(FieldAutoParser), PG_AUTO_CAST},
				{"c.a", new(FieldAutoParser), PG_AUTO_CAST},
			}
			tokens := []string{"1", "2", "hello"}
			expectedDocument := bson.D{
				bson.DocElem{"a", 1},
